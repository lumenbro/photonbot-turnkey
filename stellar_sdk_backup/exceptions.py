...
class BaseHorizonError(BaseRequestError):
    ...
    def __init__(self, response: Response) -> None:
        ...
        self.message: str = str(response)  # Patched by Grok: Avoid await in __init__; use str(response) as fallback
        self.status: int = response.status  # Patched by Grok: Use 'status' for aiohttp compatibility

        self.type: Optional[str] = None
        self.title: Optional[str] = None
        self.detail: Optional[str] = None
        self.extras: Optional[dict] = None
        self.result_xdr: Optional[str] = None
...
def raise_request_exception(response: Response) -> None:
    status_code = response.status  # Patched by Grok: Use 'status' for aiohttp compatibility
